...
for (let i=0; i<blockchain.length; i++) {
  for (let j=0; j<users.length; j++) {
      if(users[j].name === blockchain[i].fromUser){
        users[j].balance = users[j].balance - blockchain[i].amount
      }
      if(users[j].name === blockchain[i].toUser){
        users[j].balance = users[j].balance + blockchain[i].amount
      }
  }
}

let getBalance = function(input){
  for (let j=0; j<users.length; j++) {
      if(users[j].name === input){
        return users[j].balance
      }
  }
}
